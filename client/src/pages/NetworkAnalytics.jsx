  ...
  const fetchNetworkData = async () => {
    try {
      ...
      const [
        ...
        api.get('/api/social-network/influential?limit=10'),
        api.get('/api/social-network/recommendations?limit=10', {
          headers: { Authorization: `Bearer ${token}` }
        }),
        api.get('/api/social-network/statistics')
      ]);

      setData({
        userMetrics: metricsResponse.data.data,
        connectionAnalysis: connectionsResponse.data.data,
        influentialUsers: influentialResponse.data.data,
        recommendations: recommendationsResponse.data.data,
        networkStats: statsResponse.data.data
      });
    } catch (error) {
